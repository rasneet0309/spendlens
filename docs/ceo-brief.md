...
**2. Live or scheduled rate updates.** Manually refreshing the static snapshot monthly is fine for now, but automating it removes a recurring manual step entirely — the original problem we set out to solve.
